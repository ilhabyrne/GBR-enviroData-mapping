...
dat <- read.csv("~/Documents/GitHub/GBR-enviroData-mapping/02_eReefs/synthesis/RRAP_enviro_metadata_targetSpp_2025-12-09.csv")
...
bgc_vars <- c(
  ...
  # Chlorophyll-a
  "Chl_a_sum_mean_monthly_max", "Chl_a_sum_median_monthly_max", "Chl_a_sum_overall_mean",
  "Chl_a_sum_overall_median", "Chl_a_sum_cv_monthly_mean", "Chl_a_sum_sd_monthly_mean",
  
  
  # Dissolved inorganic nitrogen
  "NO3_mean_monthly_max", "NO3_median_monthly_max", "NO3_overall_mean",
  "NO3_overall_median", "NO3_cv_monthly_mean", "NO3_sd_monthly_mean",
  
  # Total suspended solids
  "Oxygen_mean_monthly_max", "Oxygen_median_monthly_max", "Oxygen_overall_mean",
  "Oxygen_overall_median", "Oxygen_cv_monthly_mean", "Oxygen_sd_monthly_mean",
  
  # Secchi depth
  "PH_mean_monthly_max", "PH_median_monthly_max", "PH_overall_mean",
  "PH_overall_median", "PH_cv_monthly_mean", "PH_sd_monthly_mean"
  
  # Add more BGC variables as needed...
)

# Get all numeric variables present in the data
all_numeric <- names(dat)[sapply(dat, is.numeric)]

# Update env_vars to include BGC
env_vars <- all_numeric[all_numeric %in% c(temp_vars, wind_vars, current_vars, bgc_vars)]
...
# --- 2e. BGC-only correlation matrix ---
bgc_vars_present <- bgc_vars[bgc_vars %in% names(dat_unique)]

if(length(bgc_vars_present) > 1) {
  cor_matrix_bgc <- cor(dat_unique[, bgc_vars_present], use = "pairwise.complete.obs")
  
  png("correlation_matrix_bgc.png", width = 12, height = 12, units = "in", res = 150)
  corrplot(cor_matrix_bgc, method = "color", type = "upper",
           order = "hclust", tl.cex = 1, tl.col = "black",
           col = colorRampPalette(c("#2166AC", "#F7F7F7", "#B2182B"))(200),
           mar = c(0, 0, 2, 0))
  dev.off()
...
# --- 2f. Identify highly correlated pairs ---
...
# PCA of BGC variables only
bgc_vars_present <- bgc_vars[bgc_vars %in% names(dat_unique)]

# Check for NAs first
na_counts_bgc <- sapply(dat_unique[, bgc_vars_present], function(x) sum(is.na(x)))
na_counts_bgc[na_counts_bgc > 0]

# Run PCA on complete cases
dat_bgc_complete <- dat_unique %>%
  filter(complete.cases(dat_unique[, bgc_vars_present]))

pca_result <- prcomp(dat_bgc_complete[, bgc_vars_present], center = TRUE, scale. = TRUE)
pca_summary <- summary(pca_result)

# Extract scores
pca_scores <- as.data.frame(pca_result$x) %>%
  mutate(locality = dat_bgc_complete$locality)
...
top_loadings <- pca_loadings %>%
  mutate(loading_magnitude = sqrt(PC1^2 + PC2^2)) %>%
  slice_max(loading_magnitude, n = 10)

# Filter to focal sites
pca_scores_focal <- pca_scores %>%
  filter(!locality %in% c("Dungeness", "Tydeman", "MacGillivray",
                          "Myrmidon", "Magnetic_Island", "North_Keppel",
                          "Miall", "Middle", "Halfway", "Great_Keppel",
...
ggsave("pca_biplot_bgc.png", p_pca_bgc, width = 6, height = 5, dpi = 300)

# Plot loadings - variables in PC space
p_loadings_bgc <- ggplot(pca_loadings, aes(x = PC1, y = PC2)) +
  geom_point(size = 3, color = "#CC79A7", alpha = 0.7) +
  geom_text_repel(aes(label = variable), size = 3, max.overlaps = 20) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.3) +
  labs(x = paste0("PC1 (", round(pca_summary$importance[2,1]*100, 1), "%)"),
       y = paste0("PC2 (", round(pca_summary$importance[2,2]*100, 1), "%)"),
       title = "BGC Variable Loadings") +
  theme_bw() +
  coord_fixed()

p_loadings_bgc

ggsave("pca_loadings_bgc.png", p_loadings_bgc, width = 5, height = 6, dpi = 300)
...
cor_matrix_bgc <- cor(dat_bgc_complete[, bgc_vars_present], use = "pairwise.complete.obs")
...
# -----------------------------------------------------------------------------
# 8. BGC SPATIAL PATTERNS (new section)
# -----------------------------------------------------------------------------

# Check what BGC variables are available
bgc_vars_present <- bgc_vars[bgc_vars %in% names(dat)]

if(length(bgc_vars_present) > 0) {
  
  # Example: Chlorophyll vs latitude
  if("Chl_a_sum_overall_mean" %in% names(dat)) {
    
    bgc_site_summary <- dat %>%
      group_by(locality) %>%
      summarise(
        decimalLatitude = mean(decimalLatitude, na.rm = TRUE),
        n_samples = n(),
        Chl_a_mean = mean(Chl_a_sum_overall_mean, na.rm = TRUE),
        Chl_a_cv = mean(Chl_a_sum_cv_monthly_mean, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      filter(!locality %in% c("Dungeness", "Tydeman", "MacGillivray",
                              "Myrmidon", "Magnetic_Island", "North_Keppel",
    ...
    p_chl_lat <- ggplot(bgc_site_summary, aes(x = decimalLatitude, y = Chl_a_mean)) +
      geom_point(aes(size = n_samples, color = Chl_a_mean), alpha = 0.7) +
      geom_smooth(method = "lm", se = TRUE, color = "gray30", fill = "gray70", alpha = 0.2) +
      scale_x_reverse() +
      scale_color_viridis_c(option = "viridis", name = "Chl-a (µg/L)") +
      labs(x = "Latitude", y = "Mean Chlorophyll-a",
           size = "N samples") +
      theme_bw()
    
    ggsave("chl_a_latitude.png", p_chl_lat, width = 8, height = 6, dpi = 300)
  }
}
